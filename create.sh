...
docker build --build-arg="IMAGE_TAG=${IMAGE_TAG}" --build-arg="INSTALL_NEW_GHC=${INSTALL_NEW_GHC}" --build-arg="PKG_DB=${PKG_DB}" --build-arg="ROOT=${ROOT}" -t haskell-template-setup .
